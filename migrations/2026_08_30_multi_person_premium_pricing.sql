...
-- Fail closed if the baseline catalog required to derive the premium rates is incomplete.
DO $$
DECLARE
...
-- Exactly three participant-agnostic premium SKUs.
...
-- Clone baseline pricebook overrides and apply only the premium unit-rate uplift.
-- Multi-person rows are intentionally unbounded so a baseline max_qty cannot cap
-- a larger multi-person workload.
...
-- Variant code intentionally matches SKU code. The variant preserves each studio's
-- native quantity contract so existing preview/reserve clients do not need a
-- pricing-service translation layer.
...
-- Certification gates. Abort the transaction on any partial or proliferated catalog.
DO $$
DECLARE
  sku_count integer;
