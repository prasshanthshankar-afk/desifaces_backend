...
command -v gh >/dev/null 2>&1 || fail "GitHub CLI missing on production VM"
...
NETS="$(docker inspect -f '{{range $k,$v := .NetworkSettings.Networks}}{{println $k}}{{end}}' "$CONTAINER" | sed '/^$/d')"
[[ "$NETS" == "$NETWORK" ]] || fail "unexpected Web network: $NETS"
...
echo "============================================================"
echo " desifaces — PROD WEB HOTFIX DEPLOY"
echo " source=PRIVATE_GITHUB_VIA_AUTHENTICATED_GH"
...
rm -rf "$TMP"
gh repo clone "$WEB_REPO" "$TMP" -- --filter=blob:none --no-checkout --quiet
git -C "$TMP" fetch --quiet origin "$TARGET_SHA" --depth=1
