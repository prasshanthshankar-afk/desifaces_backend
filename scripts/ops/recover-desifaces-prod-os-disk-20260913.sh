...
echo "============================================================"
echo " desifaces — PRODUCTION OS DISK RECOVERY"
...
[[ "${CURRENT_OS_ID,,}" == "${WRONG_OS_DISK_ID,,}" ]] || fail "current OS disk is not the expected 30-GB recovery/non-prod disk"
...
PROD_SIZE="$(az disk show --ids "$PROD_DISK_ID" --query diskSizeGb -o tsv)"
CURRENT_SIZE="$(az disk show --ids "$WRONG_OS_DISK_ID" --query diskSizeGb -o tsv)"
[[ "$PROD_SIZE" =~ ^[0-9]+$ ]] || fail "invalid production disk size"
[[ "$CURRENT_SIZE" =~ ^[0-9]+$ ]] || fail "invalid current OS disk size"
...
PROD_DES="$(az disk show --ids "$PROD_DISK_ID" --query diskEncryptionSetId -o tsv)"
CURRENT_DES="$(az disk show --ids "$WRONG_OS_DISK_ID" --query diskEncryptionSetId -o tsv)"
...
echo "swap the VM back to the persistent production OS disk, start the VM, and certify the recovered stack."
...
[[ "$CURRENT_SIZE" == "$PROD_SIZE" ]] || fail "OS disk sizes do not match after compatibility step"
