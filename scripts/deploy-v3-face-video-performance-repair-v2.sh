...
RUN="/tmp/v3-face-video-performance-${STAMP}"
BWT="$RUN/backend"
WWT="$RUN/web"
...
for x in git docker curl python3; do command -v "$x" >/dev/null 2>&1 || fail "missing command: $x"; done
...
RUNTIME_CHANGED=0
FUSION_WORKER_STOPPED=0
WEB_CHANGED=0
WEB_IMAGE="$(docker inspect -f '{{.Config.Image}}' "$WEB_CONTAINER")"
WEB_ROLLBACK_TAG="desifaces-web-before-face-video-performance:${STAMP}"

restore_file(){
  local container="$1" rel="$2" snap="$RUN/rollback/$container/${rel//\//__}"
  [[ -f "$snap" ]] && docker cp "$snap" "$container:/app/$rel" >/dev/null 2>&1 || true
}
cleanup(){
  set +e
...
rollback(){
  ...
  if (( rc != 0 )); then
    ...
    if (( WEB_CHANGED == 1 )) && docker image inspect "$WEB_ROLLBACK_TAG" >/dev/null 2>&1; then
      ...
      log "ROLLBACK: web restored"
    fi
    if (( RUNTIME_CHANGED == 1 )); then
      restore_file "$FUSION_WORKER" app/workers/fusion_worker.py
      for c in "$FACE_API" "$FACE_WORKER"; do
        restore_file "$c" app/domain/models.py
        restore_file "$c" app/services/creator_orchestrator.py
      done
      restore_file "$EXT_API" app/api/routes/longform.py
      docker restart "$FUSION_WORKER" "$FACE_API" "$FACE_WORKER" "$EXT_API" >/dev/null 2>&1 || true
      log "ROLLBACK: modified backend runtime restored"
    elif (( FUSION_WORKER_STOPPED == 1 )); then
      docker start "$FUSION_WORKER" >/dev/null 2>&1 || true
    fi
...
log "============================================================"
log " desifaces V3 — FACE + VIDEO PERFORMANCE REPAIR v2"
log "============================================================"
log "run=$RUN"
...
if [[ -n "$(git status --porcelain)" ]]; then
  ...
  git -c user.name='desifaces release' -c user.email='release@desifaces.ai' commit \
    -m 'fix(v3): remove core Fusion serialization and expose partial Face results' >/dev/null
fi
...
if ! docker compose -f docker-compose.web.yml build desifaces-web >"$WEB_BUILD_LOG" 2>&1; then
  ...
  fail "web certification/build failed"
fi
if [[ -n "$(git status --porcelain --untracked-files=normal | grep -vE '^(\?\?| M) (web/.env|control-plane/.env)$' || true)" ]]; then
  ...
  git -c user.name='desifaces release' -c user.email='release@desifaces.ai' commit \
    -m 'fix(web): show all Face outputs and partial completion truthfully' >/dev/null
fi
...
for c in "$FACE_API" "$FACE_WORKER"; do
  docker cp "$c":/app/app/domain/models.py "$RUN/rollback/$c/app__domain__models.py"
  docker cp "$c":/app/app/services/creator_orchestrator.py "$RUN/rollback/$c/app__services__creator_orchestrator.py"
done
...
log ""
log "===== 4. DRAIN CURRENT CORE FUSION WORK ====="
ACTIVE=0
...
PY
  ...
  [[ "$ACTIVE" == 0 ]] && break
  if (( attempt == 1 || attempt % 4 == 0 )); then log "waiting_for_active_core_fusion_jobs=$ACTIVE attempt=$attempt/80"; fi
  sleep 15
done
[[ "$ACTIVE" == 0 ]] || fail "core Fusion jobs did not drain within 20 minutes; no runtime mutation performed"
# Stop immediately after the zero-active gate so the old worker cannot claim a
# new queued child between the drain check and the file replacement.
docker stop -t 20 "$FUSION_WORKER" >/dev/null
FUSION_WORKER_STOPPED=1
log "CORE_FUSION_DRAIN_AND_STOP=PASS"
...
for c in "$FACE_API" "$FACE_WORKER"; do
  docker cp "$BWT/services/svc-face/app/app/domain/models.py" "$c":/app/app/domain/models.py
...
docker start "$FUSION_WORKER" >/dev/null
FUSION_WORKER_STOPPED=0
docker restart "$FACE_API" "$FACE_WORKER" "$EXT_API" >/dev/null
log "UPDATED=$FUSION_WORKER,$FACE_API,$FACE_WORKER,$EXT_API"
...
for _ in $(seq 1 45); do
  ...
  worker_state="$(docker inspect -f '{{.State.Status}}' "$FUSION_WORKER" 2>/dev/null || true)"
  if [[ "$face_code" == 200 && "$ext_ok" == *ok* && "$worker_state" == running ]]; then READY=1; break; fi
  sleep 2
done
(( READY == 1 )) || fail "modified runtime did not become ready"
...
assert not hasattr(JobStatus, 'PARTIAL_SUCCESS')
s=inspect.getsource(CreatorOrchestrator)
assert 'FACE_PARTIAL_RESULT_V1' in s
assert 'actual_units=completed_count' in s
assert '"partial_success": partial_result' in s
print('FACE_PARTIAL_RESULT_RUNTIME=PASS backward_compatible_status=true')
PY

docker exec -i "$EXT_API" python - <<'PY'
from app.api.routes.longform import _longform_progress
p=_longform_progress(
    {'status':'running','total_segments':2,'created_at':None},
    [{'status':'video_running'},{'status':'provider_running'}],
)
...
for spec in Audio:18004 Pricing:18009 Director:18011; do
  name="${spec%%:*}"; port="${spec##*:}"
...
log ""
log "============================================================"
log " FACE + VIDEO PERFORMANCE REPAIR: DEPLOYED + CERTIFIED"
log "============================================================"
log "backend_sha=$BACKEND_SHA"
log "web_sha=$WEB_SHA"
log "core_fusion_worker_concurrency=4_default_bounded_8"
log "provider_submissions=truly_parallel_after_core_fusion_queue"
log "face_partial_result=visible_without_new_backend_status"
log "face_completed_variants=preserved_and_charged_only"
log "face_gallery=all_completed_variants_clickable"
log "video_progress=completed-output-weighted"
log "pricing=unchanged"
log "reservation_lifecycle=unchanged"
log "provider_routing=unchanged_kling"
log "kling_provider_latency=not_fixed_by_this_patch"
log "next_provider_action=benchmark_heygen_vs_kling"
log "rollback_dir=$RUN/rollback"
log "============================================================"
