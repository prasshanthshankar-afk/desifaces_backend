...
CERT="${CERT:-/etc/letsencrypt/live/dev-api.desifaces.ai/fullchain.pem}"
KEY="${KEY:-/etc/letsencrypt/live/dev-api.desifaces.ai/privkey.pem}"
...
for x in sudo nginx curl openssl python3 grep cp; do need "$x"; done
...
code="$(curl -sS -o /tmp/df-v3-bridge-local.html -w '%{http_code}' "$TARGET/auth/login")"
...
[[ -f "$CERT" && -f "$KEY" ]] || fail "expected existing TLS material is missing"
openssl x509 -in "$CERT" -noout -ext subjectAltName 2>/dev/null | grep -Fq "DNS:$HOST" || fail "$CERT does not cover $HOST"
log "DEV_APP_TLS_SAN=PASS"

mapfile -t FILES < <(sudo grep -RIl -E "server_name[^;]*${HOST//./\\.}" /etc/nginx/sites-enabled /etc/nginx/conf.d 2>/dev/null | sort -u)
((${#FILES[@]} == 1)) || fail "expected exactly one active nginx file containing $HOST; found ${#FILES[@]}"
CONFIG="${FILES[0]}"
BACKUP="${CONFIG}.pre-v3-web-bridge-${STAMP}"
...
code="$(curl -sS --resolve "$HOST:443:127.0.0.1" -o /tmp/df-v3-bridge-https.html -w '%{http_code}' "https://$HOST/auth/login")"
