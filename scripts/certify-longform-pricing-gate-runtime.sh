...
echo "============================================================"
echo " desifaces V3 — LONGFORM PRICING GATE RUNTIME CERTIFICATION"
echo "============================================================"
...
# Real health request executed inside the running API container. Use python -c so
# no stdin/TTY behavior can turn this into a false-positive probe.
docker exec "$EXT_API" python -c "import urllib.request; r=urllib.request.urlopen('http://127.0.0.1:8000/api/health', timeout=5); assert r.status == 200; print('FUSION_EXTENSION_HEALTH=PASS HTTP_200')"
...
# Runtime import + ordering assertions. -i is mandatory because Python reads the
# assertion program from stdin.
...
# Failure handling occurs after reserve attempt but before any segment creation.
p_block = src.index('failed_status = "blocked"')
assert p_reserve < p_block < p_insert, (p_reserve, p_block, p_insert)
...
# Confirm current wallet state was not changed by deployment/certification.
docker exec -i "$EXT_API" python - <<'PY'
import asyncio, json
...
async def main():
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            select j.id::text as job_id, j.user_id::text as user_id, j.status,
                   coalesce((v.lots_json::jsonb->>'total_spendable')::numeric,0) as spendable,
                   coalesce((v.lots_json::jsonb->>'total_reserved')::numeric,0) as reserved
        ...
        if row:
            print('ACCOUNT_STATE=' + json.dumps({
                ...
                'latest_job_status': row['status'],
                'spendable': str(row['spendable']),
