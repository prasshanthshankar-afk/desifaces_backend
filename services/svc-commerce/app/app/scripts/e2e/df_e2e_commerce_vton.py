...
def _env_bool(name: str, default: bool = False) -> bool:
    v = (_env_str(name, "") or "").strip().lower()
    if not v:
        return default
...
def _http_json(
    ...
    *,
    ...
    if data is not None and m not in ("GET", "HEAD"):
    ...
    try:
        ...
        try:
            return int(e.code), dict(e.headers.items()) if e.headers else {}, json.loads(txt) if txt else {}
        except Exception:
            return int(e.code), dict(e.headers.items()) if e.headers else {}, txt
...
def _pick_paths_from_openapi(openapi: Dict[str, Any]) -> Dict[str, str]:
    ...
    if "quote_path" not in out:
        out["quote_path"] = best("POST", ["commerce", "quote"]) or best("POST", ["quote"]) or "/api/commerce/quote"
    if "confirm_path" not in out:
        out["confirm_path"] = best("POST", ["commerce", "confirm"]) or best("POST", ["confirm"]) or "/api/commerce/confirm"
    if "status_path" not in out:
        out["status_path"] = (
            best("GET", ["commerce", "jobs", "status"], must_have_param=True)
            or best("GET", ["jobs", "status"], must_have_param=True)
...
def _is_saree_kind(outfit_kind: str) -> bool:
    k = (outfit_kind or "").strip().lower()
    return k in ("saree", "saree_set", "saree+blouse", "sari", "saari")
...
def _build_quote_body(
    *,
    ...
    human_url: str,
    garment_url: str,
    # saree-set extensions (optional; ignored for non-saree)
    outfit_kind: str = "",
    garment_type: str = "",
    drape_style: str = "",
    blouse_url: str = "",
    jewelry_earrings_url: str = "",
    jewelry_necklace_url: str = "",
    jewelry_bangles_url: str = "",
    full_body: bool = False,
    shot: str = "",
) -> Dict[str, Any]:
    """
    IMPORTANT: This must match CommerceQuoteIn exactly.
    DO NOT wrap in {"request": ...} or {"quote_request": ...}.
    """
    saree_mode = _is_saree_kind(outfit_kind)

    if saree_mode:
        gt = (garment_type or "dresses").strip().lower()
        if gt not in ("upper_body", "lower_body", "dresses"):
            gt = "dresses"
        ds = (drape_style or "nivi").strip().lower()

        items: List[Dict[str, Any]] = [
            {
                "component_code": "saree",
                "kind": "garment",
        ...
        if blouse_url:
            items.append(
                {
                    "component_code": "blouse",
                    "kind": "garment",
                    "image_url": blouse_url,
                    "image_urls": [],
                    "is_primary": False,
                    "dominance_rank": 1,
                    "meta": {"source": "e2e"},
        ...
        if jewelry_earrings_url:
            items.append({"component_code": "jewelry_earrings", "kind": "accessory", "image_url": jewelry_earrings_url, "meta": {"source": "e2e"}})
        if jewelry_necklace_url:
            items.append({"component_code": "jewelry_necklace", "kind": "accessory", "image_url": jewelry_necklace_url, "meta": {"source": "e2e"}})
        if jewelry_bangles_url:
            items.append({"component_code": "jewelry_bangles", "kind": "accessory", "image_url": jewelry_bangles_url, "meta": {"source": "e2e"}})

        # Provider still expects garment_image_url (we pass saree URL as primary garment)
        product_assets: Dict[str, Any] = {
            "product_type": product_type,
            "outfit_kind": outfit_kind,
            "dominant_component_code": "saree",
            "garment_type": gt,
            "drape_style": ds,
        ...
        # Legacy single-garment flow (keeps existing behavior)
        product_assets = {
            "product_type": product_type,
            ...
            "items": [
                {
                    "component_code": "primary_garment",
                    "kind": "garment",
                    "image_url": garment_url,
                    "image_urls": [],
                    "is_primary": True,
    ...
    if full_body:
        model_ref["full_body"] = True
        model_ref["shot"] = (shot or "full_body").strip().lower()
    ...
    return {
        ...
        "channels": [],
        "marketplaces": [],
        "views": {"full_body": True, "half_body": True},
        "outputs": {"num_images": int(num_images), "num_videos": 0},
...
def main() -> int:
    ...
    mode = _env_str("MODE", "platform_models")
    product_type = _env_str("PRODUCT_TYPE", "apparel")
    resolution = _env_str("RESOLUTION", "hd")
    ...
    # Saree-set extensions (optional)
    outfit_kind = _env_str("OUTFIT_KIND", "")
    garment_type = _env_str("GARMENT_TYPE", "")  # for saree: dresses
    drape_style = _env_str("DRAPE_STYLE", "")    # nivi / gujarati / bengali / maharashtrian (future)
    blouse_url = _env_str("BLOUSE_IMAGE_URL", "")

    jewelry_earrings_url = _env_str("JEWELRY_EARRINGS_URL", "")
    jewelry_necklace_url = _env_str("JEWELRY_NECKLACE_URL", "")
    jewelry_bangles_url = _env_str("JEWELRY_BANGLES_URL", "")

    full_body = _env_bool("FULL_BODY", default=False)
    shot = _env_str("SHOT", "")
    ...
    if saree_mode:
        print(
            "MODE=%s PRODUCT_TYPE=%s RESOLUTION=%s NUM_IMAGES=%s OUTFIT_KIND=%s GARMENT_TYPE=%s DRAPE_STYLE=%s FULL_BODY=%s"
            % (mode, product_type, resolution, num_images, outfit_kind, (garment_type or "dresses"), (drape_style or "nivi"), full_body)
        )
    else:
    ...
    quote_body = _build_quote_body(
        ...
        jewelry_earrings_url=jewelry_earrings_url,
        jewelry_necklace_url=jewelry_necklace_url,
        jewelry_bangles_url=jewelry_bangles_url,
        full_body=full_body,
        shot=shot,
    )
    (run_dir / "quote_request.json").write_text(json.dumps(quote_body, indent=2), encoding="utf-8")

    # Keep ONE canonical body first; then a couple fallbacks for older servers
    quote_bodies = [
        quote_body,
        # Fallback 1: some earlier versions used model_ref.image_url instead of human_image_url
        {**quote_body, "model_ref": {"image_url": human_url, "meta": {"source": "e2e_fallback"}}},
        # Fallback 2: some earlier versions only read product_assets.garment_image_url and ignored items[]
        {
            **quote_body,
            "product_assets": {
                **(_as_dict(quote_body.get("product_assets"))),
                "items": [],
                "meta": {"source": "e2e_fallback_no_items"},
            },
        },
    ]
    ...
    confirm_bodies = [
        {"quote_id": quote_id},
        {"quote_id": quote_id, "idempotency_key": f"e2e:{quote_id}"},
    ...
    if isinstance(status_out, dict):
        status = str(status_out.get("status") or "")
        stage = str(status_out.get("stage") or "")
        if isinstance(status_out.get("computed"), dict):
            computed = status_out["computed"]
            if not stage:
                stage = str(computed.get("stage") or "")
        elif isinstance(status_out.get("payload_json"), dict):
    ...
    if stage != "succeeded" and status.lower() != "succeeded":
        ok = False
        problems.append(f"expected succeeded, got stage={stage!r} status={status!r}")
