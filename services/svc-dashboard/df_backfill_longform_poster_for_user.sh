...
JOB_ID="$(jq -r '(.items // [])[0].source_job_id // empty' "$VIDEO_JSON")"
VIDEO_URL="$(jq -r '(.items // [])[0].reuse_payload.video_url // (.items // [])[0].video_url // (.items // [])[0].preview_url // (.items // [])[0].download_url // empty' "$VIDEO_JSON")"
