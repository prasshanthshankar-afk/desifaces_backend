    ...
    async with app.state.business_pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            select
                ...
                coalesce(dr.state::text, s.state::text, 'ready') as state,
                s.title,
                greatest(
                    coalesce(s.updated_at, s.created_at),
                    coalesce(dr.updated_at, dr.created_at, s.updated_at, s.created_at),
                    coalesce(sw.updated_at, sw.created_at, s.updated_at, s.created_at)
                ) as effective_updated_at,
                sw.workflow_id,
                sw.state::text as workflow_state,
                sw.current_stage::text as current_stage,
                case
                    when coalesce(stage_state.has_review, false) then 'awaiting_review'
                    when coalesce(stage_state.has_failed, false) then 'failed'
                    when sw.state::text in ('complete', 'completed') then 'complete'
                    when sw.state is not null then sw.state::text
                    when dr.state is not null then dr.state::text
            ...
            left join lateral (
                select r.thread_id, r.state, r.created_at, r.updated_at
            ...
            left join lateral (
                select w.workflow_id, w.state, w.current_stage, w.created_at, w.updated_at
            ...
            order by effective_updated_at desc nulls last, s.story_id
            limit $3
            """,
            auth.account_id,
            auth.user_id,
    ...
    for row in rows:
        ...
        result.append(
            RecentStoryOut(
                ...
                continue_path=f"/app/multi-person?story={story_id}",
                workflow_id=(
