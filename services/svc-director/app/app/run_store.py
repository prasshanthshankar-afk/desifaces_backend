...
class DirectorRunStore:
    ...
    async def queue_resume(
        ...
        *,
        ...
        row = await conn.fetchrow(
            ...
            set state='queued',resume_json=$4::jsonb,available_at=now(),claimed_at=null,lease_expires_at=null,last_error=null,updated_at=now()
    ...
    async def recover_expired(self, conn) -> int:
        result = await conn.execute(
            ...
            where state='running' and lease_expires_at is not null and lease_expires_at<now() and attempt_count<max_attempts
            """
        )
        try:
    ...
    async def mark_ready(
        ...
        *,
        ...
        await conn.execute(
            """
            update public.v3_director_runs
            set state='ready',project_id=$2,story_id=$3,resume_json=null,claimed_at=null,lease_expires_at=null,last_error=null,updated_at=now()
            where run_id=$1
            """,
            run_id,
            project_id,
