...
async def generate(project_id: UUID, payload: GenerateMusicIn, user=Depends(get_current_user)):
    ...
    jobs = MusicJobsRepo()
    job_id = await jobs.create_video_job(project_id=project_id, input_json=input_json)
...
async def publish(
    job_id: UUID,
    payload: PublishMusicIn = Body(default_factory=PublishMusicIn),
    user=Depends(get_current_user),
):
    #  Hard consent gate (prevents empty-body publish)
    if payload.consent.get("accepted") is not True:
