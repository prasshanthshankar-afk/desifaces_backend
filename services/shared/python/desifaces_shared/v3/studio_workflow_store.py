...
class CanonicalStudioWorkflowStore:
    ...
    async def create_workflow(self, conn, *, account_id: UUID, owner_user_id: UUID,
                              project_id: UUID, story_id: UUID | None = None,
                              metadata: dict[str, Any] | None = None,
                              workflow_id: UUID | None = None) -> UUID:
        ...
        await conn.execute(
            ...
            workflow_id, account_id, owner_user_id, project_id, story_id, metadata or {},
        )
        return workflow_id

    async def add_stage(self, conn, *, workflow_id: UUID, stage_type: StudioStageType,
                        scope_type: StudioScopeType, participant_id: UUID | None = None,
                        scene_id: UUID | None = None, dialogue_turn_id: UUID | None = None,
                        metadata: dict[str, Any] | None = None,
                        stage_run_id: UUID | None = None) -> UUID:
        ...
        await conn.execute(
            ...
            stage_run_id, workflow_id, stage_type.value, scope_type.value,
            participant_id, scene_id, dialogue_turn_id, metadata or {},
        )
        return stage_run_id
    ...
    async def add_dependency(self, conn, *, parent_stage_run_id: UUID, child_stage_run_id: UUID) -> None:
        await conn.execute(
            ...
            parent_stage_run_id, child_stage_run_id,
        )

    async def bind_generation(self, conn, *, stage_run_id: UUID,
                              generation_request_id: UUID, generation_job_id: UUID) -> None:
        await conn.execute(
            """update public.v3_studio_stage_runs
            set generation_request_id=$2,generation_job_id=$3,updated_at=now()
            where stage_run_id=$1""",
            stage_run_id, generation_request_id, generation_job_id,
        )
    ...
    async def attach_output(self, conn, *, stage_run_id: UUID,
        ...
        await conn.execute(
            """insert into public.v3_studio_stage_outputs(stage_run_id,media_id,output_role,is_active)
            values($1,$2,$3,true)
            on conflict(stage_run_id,media_id,output_role)
            do update set is_active=true""",
            stage_run_id, media_id, output_role,
        )
        review_id = uuid4()
        await conn.execute(
            """insert into public.v3_studio_review_items(review_item_id,stage_run_id,media_id,decision)
            values($1,$2,$3,'pending') on conflict(stage_run_id,media_id) do nothing""",
            review_id, stage_run_id, media_id,
        )
        await conn.execute(
        ...
        row = await conn.fetchrow(
            ...
            stage_run_id, media_id,
        )
        return UUID(str(row["review_item_id"]))

    async def review_output(self, conn, *, review_item_id: UUID, reviewer_user_id: UUID,
                            decision: ReviewDecision, feedback: str | None = None) -> UUID:
        ...
        row = await conn.fetchrow(
            ...
            where review_item_id=$1 returning stage_run_id,media_id""",
            review_item_id, decision.value, reviewer_user_id, feedback,
        )
        if not row:
        ...
        if decision in {ReviewDecision.REJECTED, ReviewDecision.REVISE}:
            await conn.execute(
                """update public.v3_studio_stage_outputs set is_active=false
                where stage_run_id=$1 and media_id=$2""",
                stage_run_id, media_id,
            )

        active_total = int(await conn.fetchval(
            "select count(*) from public.v3_studio_stage_outputs where stage_run_id=$1 and is_active=true",
            stage_run_id,
        ) or 0)
        active_unapproved = int(await conn.fetchval(
            ...
            where o.stage_run_id=$1 and o.is_active=true and coalesce(r.decision,'pending')<>'approved'""",
            stage_run_id,
        ) or 0)

        if active_total > 0 and active_unapproved == 0:
            await conn.execute(
    ...
    async def bind_approved_input(self, conn, *, stage_run_id: UUID, media_id: UUID,
                                  input_role: str, source_stage_run_id: UUID) -> None:
        await conn.execute(
    ...
    async def set_workflow_state(self, conn, *, workflow_id: UUID, state: StudioWorkflowState,
                                 current_stage: StudioStageType | None = None,
                                 final_media_id: UUID | None = None) -> None:
        await conn.execute(
            """update public.v3_studio_workflows
            set state=$2,current_stage=$3,final_media_id=coalesce($4,final_media_id),updated_at=now()
            where workflow_id=$1""",
            workflow_id, state.value, current_stage.value if current_stage else None, final_media_id,
        )
    ...
    async def _stage_view(self, conn, row) -> StudioStageView:
        ...
        outputs = await conn.fetch(
            "select media_id,output_role,is_active from public.v3_studio_stage_outputs where stage_run_id=$1 order by created_at,media_id",
            row["stage_run_id"],
        )
        ...
        return StudioStageView(
            stage_run_id=UUID(str(row["stage_run_id"])), workflow_id=UUID(str(row["workflow_id"])),
            stage_type=StudioStageType(str(row["stage_type"])), scope_type=StudioScopeType(str(row["scope_type"])),
            ...
            inputs=tuple(StudioArtifactRef(
                media_id=UUID(str(item["media_id"])), role=str(item["input_role"]),
            ...
            outputs=tuple(StudioArtifactRef(
                media_id=UUID(str(item["media_id"])), role=str(item["output_role"]), is_active=bool(item["is_active"]),
            ) for item in outputs),
            reviews=tuple(StudioReviewItem(
                review_item_id=UUID(str(item["review_item_id"])), stage_run_id=UUID(str(item["stage_run_id"])),
                media_id=UUID(str(item["media_id"])), decision=ReviewDecision(str(item["decision"])),
                reviewer_user_id=UUID(str(item["reviewer_user_id"])) if item["reviewer_user_id"] else None,
                feedback=item["feedback"], decided_at=item["decided_at"],
            ) for item in reviews),
