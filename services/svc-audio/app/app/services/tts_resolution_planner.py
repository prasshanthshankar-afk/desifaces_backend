...
class TTSResolutionPlanner:
    ...
    async def resolve(
        ...
        try:
            resolved_locale = await self.locale_resolver.resolve(
                raw_locale
            )
        ...
        # LocaleResolver returns a ResolvedLocale domain object.
        #
        # Do NOT stringify that object. The provider/model catalog
        # requires its canonical BCP-47 locale field.
        canonical_locale = str(
            getattr(
                resolved_locale,
                "locale",
        ...
        # Optional side-by-side context refinement.
        #
        # Exact regional locales are authoritative and are never
        # rewritten by country/region context. Generic locales may
        # be refined only through SQL-backed context rules.
        if self.context_resolver is not None:
            try:
                context_resolution = (
                    await self.context_resolver.resolve(
        ...
        return TTSResolutionPlan(
            ...
            provider_locale_code=(
                model.provider_locale_code
            ),
            provider_language_code=(
                model.provider_language_code
            ),
            ...
            routing_policy_code=(
                model.routing_policy_code
            ),
            masterdata_revision=(
                model.masterdata_revision
            ),
